...
my @barcodes = get_file_data ("/opt/local/scripts/bin/barcodes.txt");
chomp @barcodes;
...
open my $FILEA, "< ".$ResultDirectory."/".$filenameA;

$filenameA =~ s/.fasta//g;
my $filenameOut = $filenameA.$barcodes[$tag_number].".fasta";

open my $OUTFILE, "> ".$ResultDirectory."/".$filenameOut;
...
WriteArrayToFile ($ResultDirectory."/".$filenameA.".map", @mapping);

#return ($barcodes[$tag_number]);
